...
const ProductCard = ({ data }) => {
  ...
  const productNameSlug = data.name
    ...
    <div className="max-w-sm mx-auto bg-white dark:bg-zinc-800 rounded-xl shadow-md overflow-hidden md:max-w-2xl transform transition-transform duration-300 hover:scale-105 relative">
      <div className="relative aspect-w-1 aspect-h-1">
        ...
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2">
          <button
            className="bg-white dark:bg-zinc-700 p-2 rounded-full shadow-md opacity-75 hover:opacity-100"
            onClick={handleAddToCart}
          >
            <FontAwesomeIcon icon={faCartShopping} />
          </button>
          <Link
            to={`/${data.category_id}/${data.id}/${productNameSlug}`}
            className="bg-white dark:bg-zinc-700 p-2 rounded-full shadow-md opacity-75 hover:opacity-100"
          >
      ...
      <div className="p-4">
        <div className="text-blue-500 dark:text-blue-300 text-sm font-semibold">
          {data.category}
        </div>
        <div className="flex items-center justify-between">
          <Link
            to={`/${data.category_id}/${data.id}/${productNameSlug}`}
            className="text-lg font-bold text-zinc-900 dark:text-gray-300"
          >
            {data.name}
          </Link>
        ...
        <div className="flex items-center mt-4">
          ...
          <span className="ml-2 text-green-500 dark:text-orange-400 font-semibold">
            ${data.price}
          </span>
        </div>
      </div>
